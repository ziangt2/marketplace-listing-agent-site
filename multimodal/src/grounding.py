...
from .product_evidence import TEXT_FIELDS, DIMENSIONS, inches

PROMPT_VERSION = "abo_grounded_selection_v2"
...
attributes, prices, ratings, availability, or claims about room suitability. Copy dimensions from
the candidate's fields dictionary, which is normalized to inches, not from title or dimension_sources.
Prefer informative
...
def validate_output(raw, evidence, catalog_ids):
    ...
    for rec in raw["recommendations"]:
        ...
        for claim in claims:
            ...
            equivalent = isinstance(claim["value"], str) and field in fields and claim["value"] == fields[field]
            if field in DIMENSIONS and field in fields:
                try:
                    equivalent = abs(inches(claim["value"]) - inches(fields[field])) <= 1e-6
                except ValueError:
                    equivalent = False
            if equivalent:
                counts["value_valid"] += 1
                canonical_claims.append({"field": field, "value": fields[field]})
            else:
        ...
        if len(errors) == before:
            accepted.append({"product_id": pid, "evidence": canonical_claims})
