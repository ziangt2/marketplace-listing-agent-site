...
PLAN_VERSION = "abo_search_plan_v1"
    ...
    def run(self, query, image_path=None):
        ...
        try:
            ...
            for field in ("search_query", "broad_query"):
                if not isinstance(plan[field], str) or not plan[field].strip() or len(plan[field]) > 2000:
            ...
            route = obvious_route(query, bool(image_path)) or plan["route"]
            plan = dict(plan, selected_tool=route, routing_policy="explicit-modality-and-lexical-preference-v1")
